...
class zhuanti(CrawlSpider):
    ...
    def parse(self, response):
        ...
        for info in infos:
            try:
                name = info.xpath('div/a/h4/text()').extract()
                content = info.xpath('div/a[1]/p/text()').extract()
                article = info.xpath('div/div/a/text()').extract()
                fans = info.xpath('div/div/text()').extract()
